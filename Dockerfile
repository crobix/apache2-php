##
# Loïc AUDU PHP 7.4 + Apache 2 container
##

FROM        	crobix/php:php74
...
# Apache
RUN		apt-get -y install apache2 libapache2-mod-php7.4
RUN		a2enmod rewrite
RUN		a2enmod headers
...
RUN		  sed -i 's/post_max_size = 8M/post_max_size = 50M/g' /etc/php/7.4/apache2/php.ini
RUN     sed -i 's/upload_max_filesize = 2M/upload_max_filesize = 50M/g' /etc/php/7.4/apache2/php.ini
RUN     sed -i 's/\;\ max_input_vars\ \=\ 1000/max_input_vars\ \=\ 250000/g' /etc/php/7.4/apache2/php.ini
...
RUN     sed -i 's/\output_buffering\ \=\ 4096/output_buffering\ \=\ Off/g' /etc/php/7.4/apache2/php.ini
RUN     sed -i 's/\session.cookie_httponly\ \=/session.cookie_httponly\ \=\ On/g' /etc/php/7.4/apache2/php.ini

RUN     phpenmod -v 7.4 -s apache2 sqlsrv pdo_sqlsrv amqp xdebug redis imagick
